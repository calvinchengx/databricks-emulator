...
| Feature | Emulator | Type |
|---|---|---|
| Jobs 2.2 — notebook / Python | Shim resolves workspace/DBFS file, bakes argv / notebook params / `{{secrets}}` into a Python preamble (`os.environ.update`), POSTs `{agent}/statements` with `kind: python`. Without `DATABRICKS_SPARK_CONNECT_URL`, `run-now` fails naming the engine — never `SUCCESS`. Default attach is Sail behind the family's spark-agent (`make e2e-engine`). Family compose does not set the URL. **Argv and env are per-task, including tasks that overlap.** The preamble writes `sys.argv` and `os.environ`, one object each per interpreter, and `runJob` dispatches a wave concurrently — so two parameterised tasks in one wave used to overwrite each other and both read the winner's, reporting `SUCCESS` either way. Fixed agent-side in fabric-emulator (`python/spark_agent/task_scope.py`, shipped in the pinned digest) and **witnessed by `ci:e2e-task-parameters`**: two parameterised tasks in ONE wave, with delta-rs confirming each observed its own parameters and its own environment. A serial suite cannot see this either way, which is why it is its own witness. | 🟢 Real |
...
| Jobs 2.2 — `dbt_task` | The project is read from the workspace store, carried INLINE to the statement agent (the store is this process's disk; the agent is another container, so a path meaningful here is meaningless there), and dbt is invoked through `dbtRunner` against a generated profile pointing at the `warehouse_id` the task named and **filed under the name the project's own `dbt_project.yml` declares**, so a project runs here without being edited to say an emulator-specific profile name. dbt is an ordinary warehouse client, so running it as a job changes who invokes it, not what it connects to. `source: GIT` is refused by name: clone with the Repos API and point `project_directory` at the result. An unknown warehouse, a directory with no `dbt_project.yml`, and a project over 8 MiB all fail naming the cause. The profile names this emulator as **the agent** reaches it (`-agent-url`, defaulting to `-public-url`), because a client-facing address like `127.0.0.1:18470` resolves inside the agent to the agent itself. A dbt failure comes back as a FIELD beside the artefacts rather than as an exception, so an agent that closes the connection on `SystemExit` cannot turn a failing test into a transport error and lose `run_results.json` with it. **`ci:e2e-dbt-task` runs a failing test**, not only passing models: it requires the run to fail for dbt's reason rather than the transport's, and requires `run_results.json` to survive naming the test — the two properties were held by Go tests alone while the gate exercised neither. The task's `spark_env_vars` are set INSIDE the generated code before dbt parses, so `env_var()` in a project's `sources.yml` resolves; the request carries no `env` field at all, because the agent never read one and a second, inert copy only made the field look supported. Needs `dbt-databricks` on the agent; an older agent fails saying exactly that. | 🟢 Real |
