...
func (s *Server) Handler() http.Handler {
	...
	mux.HandleFunc("POST /api/2.0/sql/queries", s.protect(s.sqlCreateQuery))
	mux.HandleFunc("GET /api/2.0/sql/queries", s.protect(s.sqlListQueries))
	mux.HandleFunc("GET /api/2.0/sql/queries/{id}/visualizations", s.protect(s.sqlQueryVisualizationsRefused))
	mux.HandleFunc("GET /api/2.0/sql/queries/{id}", s.protect(s.sqlGetQuery))
	mux.HandleFunc("PATCH /api/2.0/sql/queries/{id}", s.protect(s.sqlUpdateQuery))
	mux.HandleFunc("DELETE /api/2.0/sql/queries/{id}", s.protect(s.sqlDeleteQuery))
	mux.HandleFunc("GET /api/2.0/sql/history/queries", s.protect(s.sqlListQueryHistory))
	mux.HandleFunc("/api/2.0/sql/alerts", s.protect(s.sqlAlertsRefused))
	mux.HandleFunc("/api/2.0/sql/alerts/", s.protect(s.sqlAlertsRefused))
	mux.HandleFunc("/api/2.0/preview/sql/alerts", s.protect(s.sqlAlertsRefused))
	mux.HandleFunc("/api/2.0/preview/sql/alerts/", s.protect(s.sqlAlertsRefused))
	mux.HandleFunc("/api/2.0/alerts", s.protect(s.sqlAlertsRefused))
	mux.HandleFunc("/api/2.0/alerts/", s.protect(s.sqlAlertsRefused))
